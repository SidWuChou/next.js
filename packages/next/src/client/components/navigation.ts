...
export function useSearchParams(): ReadonlyURLSearchParams {
  ...
  if (typeof window === 'undefined' && readonlySearchParams) {
    return instrumentReadonlySearchParamsForValidation(readonlySearchParams)
  }
...
function instrumentReadonlySearchParamsForValidation(
    ...
    switch (workUnitStore.type) {
      case 'validation-client': {
        if (workUnitStore.validationSamples) {
          const { createExhaustiveURLSearchParams } =
          ...
          return createExhaustiveURLSearchParams(
