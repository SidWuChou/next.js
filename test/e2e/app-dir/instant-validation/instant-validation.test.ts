  ...
  describe.each(cases)('$description', ({ isClientNav }) => {
    ...
    it('valid - static prefetch - suspense around runtime and dynamic', async () => {
      if (isNextDev) {
        ...
        await prerender('/suspense-in-root/static/suspense-around-dynamic')
        const output = getCliOutputSinceMark()
        expect(output).not.toContain('Build-time instant validation failed')
      }
    })
    ...
    it('valid - runtime prefetch - suspense only around dynamic', async () => {
      if (isNextDev) {
        ...
        await prerender('/suspense-in-root/runtime/suspense-around-dynamic')
        const output = getCliOutputSinceMark()
        expect(output).not.toContain('Build-time instant validation failed')
      }
    })
    ...
    it('valid - target segment not visible in all navigations', async () => {
      if (isNextDev) {
        ...
        await prerender('/default/static/valid-blocked-children')
        const output = getCliOutputSinceMark()
        expect(output).not.toContain('Build-time instant validation failed')
      }
    })
    ...
    it('invalid - static prefetch - suspense too high', async () => {
      if (isNextDev) {
        ...
        try {
          await prerender('/suspense-in-root/static/suspense-too-high')
        } catch {}
        const output = getCliOutputSinceMark()
        expect(output).toContain(
    ...
    it('invalid - runtime prefetch - suspense too high', async () => {
      if (isNextDev) {
        ...
        try {
          await prerender('/suspense-in-root/runtime/suspense-too-high')
        } catch {}
        const output = getCliOutputSinceMark()
        expect(output).toContain(
    ...
    it('invalid - runtime prefetch - sync IO after runtime API', async () => {
      if (isNextDev) {
        ...
        try {
          await prerender('/suspense-in-root/runtime/invalid-sync-io')
        } catch {}
        const output = getCliOutputSinceMark()
        expect(output).toContain(
    ...
    describe('blocking', () => {
      it('valid - blocking layout with unstable_instant = false is allowed to block', async () => {
        if (isNextDev) {
          ...
          await prerender('/suspense-in-root/static/blocking-layout')
          const output = getCliOutputSinceMark()
          expect(output).not.toContain('Build-time instant validation failed')
        }
      })
      ...
      it('valid - blocking page inside a static layout is allowed if the layout has suspense', async () => {
        if (isNextDev) {
          ...
          await prerender('/default/static/valid-blocking-inside-static')
          const output = getCliOutputSinceMark()
          expect(output).not.toContain('Build-time instant validation failed')
        }
      })
    ...
    // Client component tests - dev mode only (client JS doesn't execute during RSC render)
    if (isNextDev) {
    ...
    describe('disabling validation', () => {
      it('in a layout', async () => {
        if (isNextDev) {
          ...
          await prerender('/suspense-in-root/disable-validation/in-layout')
          const output = getCliOutputSinceMark()
          expect(output).not.toContain('Build-time instant validation failed')
        }
      })
      ...
      it('in a page', async () => {
        if (isNextDev) {
          ...
          await prerender('/suspense-in-root/disable-validation/in-page')
          const output = getCliOutputSinceMark()
          expect(output).not.toContain('Build-time instant validation failed')
        }
      })
